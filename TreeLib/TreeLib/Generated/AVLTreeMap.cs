...
namespace TreeLib
{
    ...
    public class AVLTreeMap<[Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] KeyType, [Payload(Payload.Value)] ValueType> :
        ...
        where KeyType : IComparable<KeyType>
    {
        ...
        private sealed class Node
        {
            ...
            [Feature(Feature.Dict)]
            public Node leftOrNull { get { return left_child ? left : Null; } }
            [Feature(Feature.Dict)]
            public Node rightOrNull { get { return right_child ? right : Null; } }
        ...
        [Storage(Storage.Object)]
        public AVLTreeMap([Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] IComparer<KeyType> comparer,uint capacity,AllocationMode allocationMode)
        {
        ...
        [Storage(Storage.Object)]
        [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)]
        public AVLTreeMap(uint capacity,AllocationMode allocationMode)
        ...
        [Storage(Storage.Object)]
        [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)]
        public AVLTreeMap([Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] IComparer<KeyType> comparer)
        ...
        [Storage(Storage.Object)]
        public AVLTreeMap(AVLTreeMap<KeyType, ValueType> original)
        {
            ...
            if (original.root != Null)
            {
...
#endif
                {
                    ...
                    while (nodeOriginal != Null)
                    {
                        ...
                        /*[Feature(Feature.Dict)]*/
                        nodeChild.right = nodeThis; // forward thread
                    ...
                    while (stack.Count != 0)
                    {
                        ...
                        if (nodeOriginal.right_child)
                        {
                            Node nodeChild = new Node();
                            /*[Feature(Feature.Dict)]*/
                            nodeChild.left = nodeThis; // back thread
                            /*[Feature(Feature.Dict)]*/
                            nodeChild.right = nodeControllingSuccessorThis; // forward thread

                            /*[Feature(Feature.Dict)]*/
                            nodeThis.right_child = true;
                            nodeThis.right = nodeChild;

                            Node nodeControllingPredecessor = Null;
                            /*[Feature(Feature.Dict)]*/
                            ...
                            while (nodeOriginal != Null)
                            {
                                nodeChild = new Node();
                                /*[Feature(Feature.Dict)]*/
                                nodeChild.left = nodeControllingPredecessor; // back thread
                                /*[Feature(Feature.Dict)]*/
                                nodeChild.right = nodeThis; // forward thread
        ...
        private bool NearestLess(
            out Node nearestNode,            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] KeyType key,            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] out KeyType nearestKey,            bool orEqual)
        {
            unchecked
            {
                Node lastLess = Null;
        ...
        private bool g_tree_insert_internal(
            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] KeyType key,            [Payload(Payload.Value)] ValueType value,            bool add,            bool update,            [Feature(Feature.Dict, Feature.Rank)][Payload(Payload.Value)]UpdatePredicate<KeyType, ValueType> predicateMap)
        {
            unchecked
            {
                if (root == Null)
                ...
                if (addleft)
                {
                    ...
                    /*[Feature(Feature.Dict)]*/
                    child.left = node.left; // back thread
                    /*[Feature(Feature.Dict)]*/
                    child.right = node; // forward thread
                    node.left = child;
                    /*[Feature(Feature.Dict)]*/
                    node.left_child = true;
                    node.balance--;
                ...
                else
                {
                    ...
                    /*[Feature(Feature.Dict)]*/
                    child.right = node.right; // forward thread
                    /*[Feature(Feature.Dict)]*/
                    child.left = node; // back thread
                    node.right = child;
                    /*[Feature(Feature.Dict)]*/
                    node.right_child = true;
                    node.balance++;
        ...
        private bool g_tree_remove_internal(
            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)] KeyType key,            [Feature(Feature.Dict, Feature.Rank)][Payload(Payload.Value)]UpdatePredicate<KeyType, ValueType> predicateMap)
        {
            unchecked
            {
                if (root == Null)
                ...
                if (!node.left_child)
                {
                    if (!node.right_child) // node has no children
                    {
                        ...
                        else if (left_node)
                        {
                            ...
                            /*[Feature(Feature.Dict)]*/
                            parent.left = node.left; // back thread
                            parent.balance++;
                        }
                        else
                        {
                            ...
                            /*[Feature(Feature.Dict)]*/
                            parent.right = node.right; // forward thread
                            parent.balance--;
                        }
                    }
                    else // node has a right child
                    {
                        ...
                        /*[Feature(Feature.Dict)]*/
                        successor.left = node.left; // back thread
                ...
                else // node has a left child
                {
                    if (!node.right_child)
                    {
                        /*[Feature(Feature.Dict)]*/
                        {
                            Node predecessor = g_tree_node_previous(node);
                            predecessor.right = node.right; // forward thread
                        }
                    ...
                    else // node has a both children (pant, pant!)
                    {
                        ...
                        if (successorParent != node)
                        {
                            ...
                            {
                                /*[Feature(Feature.Dict)]*/
                                successorParent.left_child = false; // 'left' remains as back thread
                            }
        ...
        public struct RobustEnumerableSurrogate : IEnumerable<EntryMap<KeyType, ValueType>>
        {
            ...
            public RobustEnumerableSurrogate(AVLTreeMap<KeyType, ValueType> tree,bool forward)
            {
                this.tree = tree;
            ...
            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)]
            public RobustEnumerableSurrogate(AVLTreeMap<KeyType, ValueType> tree,KeyType startKey,bool forward)
            {
                this.tree = tree;
                this.forward = forward;
        ...
        public struct FastEnumerableSurrogate : IEnumerable<EntryMap<KeyType, ValueType>>
        {
            ...
            public FastEnumerableSurrogate(AVLTreeMap<KeyType, ValueType> tree,bool forward)
            {
                this.tree = tree;
                this.forward = forward;
            ...
            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)]
            public FastEnumerableSurrogate(AVLTreeMap<KeyType, ValueType> tree,KeyType startKey,bool forward)
            {
                this.tree = tree;
                this.forward = forward;
            ...
            /*[Payload(Payload.Value)]*/ISetValue<ValueType>
        {
            ...
            public RobustEnumerator(AVLTreeMap<KeyType, ValueType> tree,bool forward)
            {
                this.tree = tree;
                this.forward = forward;
            ...
            [Feature(Feature.Dict, Feature.Rank, Feature.RankMulti)]
            public RobustEnumerator(AVLTreeMap<KeyType, ValueType> tree,KeyType startKey,bool forward)
            {
                this.tree = tree;
                this.forward = forward;
        ...
        public object Clone()
        {
            return new AVLTreeMap<KeyType, ValueType>(this);
        }
    }
}
