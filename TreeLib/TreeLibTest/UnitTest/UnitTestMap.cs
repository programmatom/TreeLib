...
namespace TreeLibTest
{
    public class UnitTestMap : TestBase
    {
        ...
        protected void ValidateTree<KeyType, ValueType>(
            IOrderedMap<KeyType, ValueType> tree) where KeyType : IComparable<KeyType>
        {
            ...
            if ((treeInspector = tree as INonInvasiveTreeInspection) != null)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    Console.WriteLine("[validate]: Unexpected exception occurred: {0}", exception);
                    Debug.Assert(false);
                    Debugger.Break();
                    if (throwFailure)
                    {
